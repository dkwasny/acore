...
public class AntlrPlayground {
    ...
    private static final String SDV_INPUT = "$which=${$gt($<melee>,$<spell>)}, $bonus=$?s56807[${$m3*1.4}][${$m3}], $bonus=${$m1*0.4}, $bonus=${$m1*0.5}, $chance=$?s55439[${$m2+$55439m1}][${$m2}], $charges=$?s58673[${4+2}][${4}], $damage=$?s63333[${$m1*1.15}][${$m1}]";
    ...
    public static void main(String[] args) {
        ...
        var stringThing = visitor.visitRoot(tree);

//        if (stringThing.size() != 1) {
//            throw new RuntimeException("TOO MANY STRING THINGS!");
//        }

        System.out.println("TREE: " + tree.toStringTree(parser));
        System.out.println("RESULT: " + stringThing.stream().map(x -> x.resolveString()).collect(Collectors.joining("\n")));
    }

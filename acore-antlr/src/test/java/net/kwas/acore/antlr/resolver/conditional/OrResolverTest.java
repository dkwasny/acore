...
public class OrResolverTest {
  ...
  public void bothTrue_returnsOne() {
    ...
    var resolver = new OrResolver(t, t);
    var actual = resolver.resolveNumber(ctx);
    Assertions.assertEquals(1.0, actual, 1e-9);
  }

  @Test
  public void leftTrueRightFalse_returnsOne() {
    NumberResolver t = c -> 1.0;
    NumberResolver f = c -> 0.0;
    var resolver = new OrResolver(t, f);
    var actual = resolver.resolveNumber(ctx);
    Assertions.assertEquals(1.0, actual, 1e-9);
  }

  @Test
  public void leftFalseRightTrue_returnsOne() {
    NumberResolver t = c -> 1.0;
    NumberResolver f = c -> 0.0;
    var resolver = new OrResolver(f, t);
    var actual = resolver.resolveNumber(ctx);
    Assertions.assertEquals(1.0, actual, 1e-9);
  }
  ...
  public void bothFalse_returnsZero() {
    NumberResolver f = c -> 0.0;
    var resolver = new OrResolver(f, f);
    var actual = resolver.resolveNumber(ctx);
